;(function ($) {
    ...
    $(document).ready(function () {
        /***********************************
         musemind Custom widget js
         ************************************/
        $(document).on('widget-updated',function (e,widget) {
            var widget_id = $(widget).attr('id');
            if (widget_id.indexOf('musemind_about_us_widget') != 1){
                prefetch();
            }
        });

        $('body').off('click','.musemind_flogo_uploader');

        $('body').on('click','.musemind_flogo_uploader',function (e) {
            var el = this;
            ...
            file_uploder_frame.on('insert',function () {
                ...
                var img_prev_container = $(el).siblings('.musemind-logo-preview');
                ...
                $(el).siblings().children('.musemind_logo_id').val(selected_ids.join(","));
                $(el).siblings().children('.musemind_logo_id').trigger('change');
            ...
            file_uploder_frame.on('open',function () {
                ...
                console.log($(el).siblings().children('.musemind_logo_id').val());
                var atts = $(el).siblings().children('.musemind_logo_id').val().split(",");
        ...
        function prefetch () {
            $('.musemind_logo_id').each(function () {
                var attid = $(this).val();
                var container = $(this).parent().siblings('.musemind-logo-preview');
                container.html('');

                if ( attid ){
                    $(this).parent().parent().find('.musemind_flogo_uploader').val('Change Image');
